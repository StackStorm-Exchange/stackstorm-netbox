...
class NetboxHTTPAction(NetboxBaseAction):
    ...
    def run(self, endpoint_uri, http_verb, get_detail_route_eligible, fail_non_2xx, **kwargs):
        ...
        # To maintain backward compatibility, this action always returns True (success) event when the
        # netbox http return status is not OK.  action_succeeded is set to True and will only be set
        # to False if the `fail_non_2xx` argument is set to True _and_ netbox http return code is not 2xx.
